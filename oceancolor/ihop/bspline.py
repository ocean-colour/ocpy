...
def set_a_bkpts(everyn:int=None):

    # Load for wavelengths
    ab, Rs, d_a, d_bb = ihop_io.load_loisel_2023_pca()
    wv64 = d_a['wavelength'].astype(np.float64)

    bkpts = None
    if everyn is not None:
        bkpts = [340.] + (wv64[::everyn]+2.5).tolist() + [np.max(wv64)+5.]

    if bkpts is None:
        raise IOError("Bad inputs to set_a_bkpts")
