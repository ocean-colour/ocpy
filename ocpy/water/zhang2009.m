...
4))*Kbz*Tk*IsoComp.*DFRI.^2*(6+6*delta)/(6-7*delta);
...
nsw =
n0+(n1+n2*Tc+n3*Tc^2)*S+n4*Tc^2+(n5+n6*S+n7*Tc)./lambda+n8./lambda.^2+n9./lam
bda.^3; % pure seawater
nsw = nsw.*n_air;
...
0.1323594e-6*Tc.^4+0.634575e-9*Tc.^5)./(1+21.65928e-3*Tc)*1e-6;
...
% dlnawds is partial derivative of natural logarithm of water activity
% w.r.t.salinity
% lnaw = (-1.64555e-6-1.34779e-7*Tc+1.85392e-9*Tc.^2-1.40702e11*Tc.^3)+......
% (-5.58651e-4+2.40452e-7*Tc-3.12165e-9*Tc.^2+2.40808e11*Tc.^3).*S+......
% (1.79613e-5-9.9422e-8*Tc+2.08919e-9*Tc.^2-1.39872e11*Tc.^3).*S.^1.5+......
